def main():
    ...
    min_time = [0, 0] # i = 0 (перед тобой на 2 шаг), 1 (перед тобой на 1 шаг)
    for i in range(2, N+2):
        variant_1 = one_ticket[i] + min_time[0]
        variant_2 = two_ticket[i-1] + min_time[0]
        variant_3 = three_ticket[i-2] + min_time[1]
        min_time[0], min_time[1] = min_time[1], min(variant_1, variant_2, variant_3)
